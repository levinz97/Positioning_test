...
public class LocationService extends Service {
    ...
    public IBinder onBind(Intent intent) {
        Log.i("position service","binding");
        return impl;
    }
...
//        sendBroadcastMessage(lastLocation, 0, 0);
        local_distance = 0;
        local_speed = 0;
        local_location = lastLocation;
        final ArrayList<Waypoint> waypointArrayList = new ArrayList<>();
...
//                sendBroadcastMessage(location, travelDistance, avgSpeed(travelDistance, travelTime));
    ...
    private class LocationServiceImpl extends ILocationService.Stub {
        @Override
        public void askUpdates() {
            if (local_location != null) {
                Intent intent = new Intent(ACTION_LOCATION_BROADCAST);
                intent.putExtra(EXTRA_LATITUDE, local_location.getLatitude());
                intent.putExtra(EXTRA_LONGITUDE, local_location.getLongitude());
